...
using MusicalFraction = Dani.Data.Models.Parts.MusicalFraction;
...
public class MusicalFractionConverter : JsonConverter<MusicalFraction>
{
    ...
    {
        reader.Read();
        var numerator = (byte)Math.Min(byte.MaxValue, FromElement(ref reader) ?? 0);
        reader.Read();
        var denominator = (byte)Math.Min(byte.MaxValue, FromElement(ref reader) ?? 0);
        reader.Read();
